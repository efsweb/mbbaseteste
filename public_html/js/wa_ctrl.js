...
function loadpg2(obj){
    ...
    if(typeof family === undefined || family == null){
        ...
        $.post('/modelos/setidfamily',{id: family},function(data){
            console.log(data);
            location.reload();
        });
        /*var path = location.pathname;
...
function opensearch(obj){ //pega o id e adciona no select item e chama o search() - CRIAR FUNÇAO SEGUNDARIA
    id = $(obj).attr('id');
    $('#SelectItem').attr('data-parent',id);    
    $('#popWA').popup('close');
    setTimeout(function(){
        $('#SelectItem').popup('open');
        search(); 
    },400);
    
}

function opensearchtitle(obj){ //pega o id e adciona no select item e chama o search() - CRIAR FUNÇAO SEGUNDARIA
    id = $(obj).attr('id');
    $('#SelectItem').attr('data-parent',id);
    setTimeout(function(){
        $('#SelectItem').popup('open');
...
function selectalias(obj){
    ...
    var item  = '#' + $('#SelectItem').attr('data-parent'); // PEGANDO O VALOR DE DATA-PARENT
    var tipo  = $('#param_finalidade').val();
    var a     = item.substr(-1);
    var label = $(obj).data('desc'); // PEGANDO O VALOR DE DATA-DESC
    var idlbl = $(obj).data('id');  // PEGANDO O VALOR DE DATA-ID
    
    var popup_open = $(item).attr('data-close');

    $(item).attr('data-vlr',idlbl); // PASSA O VALOR DE DATA-ID para DATA-VLR
    $(item).attr('data-tipo',tipo); // PASSA O VALOR DO SEU TIPO 'label'
    $(item).html(label);
...
function showalias(up,obj){ //DA um close no pop up anterior e executa o novo pop up
    $.mobile.loading('show');
    $('#SelectItem').popup('close');
    if(typeof up === "undefined"){
        setTimeout(function(){$('#AddAlias').popup('open');},1000);//SetTIMEout é o jogada
        $.mobile.loading('hide');
    }else{
        var x = up;
        if(up == 0){
...
function back(){
    var parent = $('#SelectItem').attr('data-parent');
    if((parent == 'popwatitulo') || (parent == 'popwadesc')){
        $('#SelectItem').popup('close');
        setTimeout(function(){$('#popWA').popup('open');},500);
    }else{
        $('#SelectItem').popup('close');
    }
}
