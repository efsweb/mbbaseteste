...
class Modelos_FichatecnicaController extends Zend_Controller_Action{
    ...
    public function geralAction(){
        ...
        $this->view->content = $modelo->loaddata($mdid,$pgid,$usuario->va[0]);
    ...
    public function dimensoesAction(){
        ...
        $modelo  = new Model_ModelosMapper();
        $this->view->content = $modelo->visaomodelos($mdid,$pgid,$usuario->va[0]);
        $this->view->pgid    = $pgid;
    }
    public function dimensoes2Action(){
        $pgid = 'mod_ft_dimensoes';
        $modeloid = new Zend_Session_Namespace('modeloselecionado');
        $mdid = $this->_getParam('id', $modeloid->id);
        if($mdid != $modeloid->id){
    ...
    public function notafinalAction(){
        $modeloid = new Zend_Session_Namespace('modeloselecionado');
        $usuario  = new Zend_Session_Namespace('usuario');
        $mdid = $this->_getParam('id', $modeloid->id);
        if($mdid != $modeloid->id){
        ...
        $this->view->content = $modelo->lista($mdid,$usuario->va[0]);
    }
    public function rodapesAction(){
        $modeloid = new Zend_Session_Namespace('modeloselecionado');
        $usuario  = new Zend_Session_Namespace('usuario');
        $mdid = $this->_getParam('id', $modeloid->id);
        if($mdid != $modeloid->id){
        ...
        $modelo = new Model_FTMapper();
        $this->view->content = $modelo->loadfooter($usuario->va[0],$mdid);
    }
    /**
    ...
    public function searchAction(){
        ...
        $modelos = new Model_ModelosMapper();
        $result  = $modelos->searchalias($param_desc_alias, $param_finalidade, $param_tipo, $param_grupo, $param_classif,$usuario->va[0]);
