...
class Model_Geralft{
    ...
    public function loaddata($idmodelo, $idpg, $idsys){
        $db     = Zend_Db_Table::getDefaultAdapter();
        $db->query('SET NAMES "utf8"');
        $db->query('SET CHARACTER SET "utf8" ');

        $arr    = array('C', $idsys, $idmodelo, '', '', '', '', '', '','');
        ...
        if(isset($teste[1])){
            ...
            if($result[0]['data_publicacao_dados'] == '14 de maio de 2015'){
                ...
                $result[0]['data_publicacao_dados'] = '';
            }
        }
        return $result;
    }

    public function atualizar($idmodelo, $complemento, $tracao, $classificacao, $aplicacao, $b09, $data, $nome_arquivo, $sys){
        $db     = Zend_Db_Table::getDefaultAdapter();
        $db->query('SET NAMES "utf8"');
        $db->query('SET CHARACTER SET "utf8" ');
        ...
        $arr    = array('U', $sys, $idmodelo, $complemento, $tracao, $classificacao, $aplicacao, $b09, $data, $nome_arquivo);
        $return = $db->query("CALL sp_fe_com_mod_ft_ctrl(?,?,?,?,?,?,?,?,?,?)", $arr);
        $result = $return->fetchAll();
        return $result;
    }
